...
Demultiplexing: `demuxalot` and `dropulation` against SNP profiles curated from DepMap release 25Q2 and CellLineProject, plus a genotype-free pass with `scSplit`. Cell lines with fewer than 100 cells were dropped from downstream analysis — which is the answer to prediction 2: **12 of 21 lines survive** into the pseudobulk DE (Fig. 6i caption, `n = 12` cell lines).
...
- **HUPO-HIPP** — the HUPO Human Immuno-Peptidome Project, the equivalent standards body for immunopeptidomics data. Its presence in the author list is why HLA evidence gets its own column in the tier table instead of being folded into "proteomics".
