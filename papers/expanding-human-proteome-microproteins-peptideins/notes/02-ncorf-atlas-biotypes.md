...
The v42 column sums to exactly 6,816. Now look at the **signs**, because that is where the information is: `intORF`, `doORF` and `lncRNA ORF` all **increase**, and no exclusion can increase a count. (Do not take comfort from the deltas summing to −448. Once the v42 counts sum to 6,816, that difference is −448 by construction — it restates the totals rather than testing them. Telling a real check from a tautological one is a skill worth practising on a paper whose ledger is otherwise this well kept.) The signs settle a question the numbers alone invite: the drop from 3,083 uORFs to 2,915 is **not** simply "the mixed ones were removed", because a pure removal cannot make `intORF`, `doORF` or `lncRNA ORF` grow. Two things happened at once and the published text does not separate them:
...
One structural observation follows, and it is worth walking through because the tempting version of it is unsound. The ORBL scheme has no processed-transcript class at all — its nine matched-null classes are `uORF`, `uoORF+1/+2`, `intORF+1/+2`, `doORF+1/+2`, `dORF` and `lncRNA-ORF` — so after re-derivation no ncORF carries a processed-transcript biotype. It is then tempting to conclude that all 291 must sit inside the 448, hence that at most `448 − 291 = 157` ORFs are excluded for the two-transcripts-two-frames reason the Methods give as their example.

That conclusion does not hold, because a class can empty by **re-typing** as well as by exclusion. A processed-transcript ORF could perfectly well have been re-typed under v42 rather than dropped — most plausibly into `lncRNA ORF`, which gained 95 members. Every ORF re-typed that way raises the 157 ceiling, and nothing in the article tells you how many were. The 'pure' biotype criteria that would settle it are in the Supplementary Results, which is not in the article PDF.

So the honest form is a **conditional** bound: *if* all 291 were excluded rather than re-typed, then at most 157 ORFs are `mixed` for the reason the Methods advertise, and the headline criterion would account for a minority of the exclusions. Absent the Supplementary criteria, that antecedent is unchecked, and the bound is worth exactly what it is worth. Note how much this resembles the Fig. 3a trap below: an inference that is arithmetically tidy and still not licensed by what the paper actually says.
