...
The corollary is what the catalogue is **not**. It is not "all ORFs in the human genome" and not "all short ORFs". For scale, when the ORBL analysis needed a null distribution it enumerated every ATG-initiated ORF of any length in any protein-coding or lncRNA transcript in GENCODE v42 that did not overlap a CDS in the same frame, applied several exclusions, and was left with **1,717,927** untranslated ORFs (Methods, ORBL). Against that background, 7,264 is a heavily filtered shortlist — one in roughly 236 candidate ORFs — selected by translation evidence. Any statistic in this paper with 7,264 in the denominator is conditioned on that filter.
