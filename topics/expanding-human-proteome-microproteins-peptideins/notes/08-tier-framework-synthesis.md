...
One clean illustration of why annotation versions are not interchangeable, since it comes up the moment anyone compares Fig. 1b with Fig. 4: the ORBL analysis re-derived biotypes on GENCODE v42 while the catalogue was built on v35. The v42 per-biotype counts sum to 6,816. It is tempting to note that the differences against Fig. 1b therefore sum to −448 and call that a reconciliation, but it is not one — 6,816 − 7,264 = −448 by construction, so the sum carries no information. The check that *does* carry information is the **sign** of the individual changes: intORF, doORF and lncRNA ORF counts all go *up*, and no pure exclusion can increase a count. So the shortfall is transcript remodelling between versions and the pure-biotype criterion superimposed, in proportions the paper never states. Never put a Fig. 1b count and a Fig. 4 fraction in the same sentence.

Note also which ORFs were even *eligible* for review: per Fig. 5e, only Tier 1A and Tier 2A ncORFs, plus Tier 1B ncORFs detected by ≥5 HLA peptides **or otherwise prioritized**, were considered for manual validation. The 77 ncORFs entering by that Tier 1B route are there because of an explicit consortium judgement that abundant HLA evidence deserves a look — and the panel's wording matters, because a separately prioritized group of 3 enters from Tier 2B by the same discretionary door.

One loose end in that panel worth knowing about, since a careful reader will spot it. The Tier 1B row shows 77 ncORFs yielding 72 peptidein annotations with a note that "4 others are protein" — which plausibly refers to the three Tier 1B ncORFs GENCODE had already annotated plus `c2riboseqorf47`. But 72 + 4 = 76 against a stated 77, so one ncORF is unaccounted for. The 121 total is unaffected. Treat the "4 others" reading as an inference and attach the residual if you use it.
